...
const Testimonials = () => {
  const { ref: testimonialsRef, inView: testimonialsVisible } = useInView<HTMLDivElement>({ threshold: 0.5 });

  return (
    <section className="testimonials-section" ref={testimonialsRef}>
      <h1 className={`testimonials-title fade-up ${testimonialsVisible ? 'show' : ''}`}>
        TESTIMONIALS
      </h1>
      <div className={`testimonial-images fade-up ${testimonialsVisible ? 'show delay-1' : ''}`}>
      ...
      <div className={`testimonials-cards fade-up ${testimonialsVisible ? 'show delay-2' : ''}`}>
