...
const NextSteps = () => {
  ...
  const { ref: nextStepsRef, inView: nextStepsVisible } = useInView<HTMLDivElement>({ threshold: 0.5 });

  return (
    <section className="next-steps" ref={nextStepsRef}>
      <div className="next-steps-content">
        <h1 className={`next-steps-heading fade-up ${nextStepsVisible ? 'show' : ''}`}>
          NEXT STEPS
        </h1>
        <p className={`next-steps-text fade-up ${nextStepsVisible ? 'show' : ''}`}>
          Ready to take the next step? Schedule a free trial or get in touch with our team!
        </p>
        <div className="next-steps-boxes">
          {/* Left box: Trial Classes - slide in from left */}
          <div className={`next-step-box slide-left ${nextStepsVisible ? 'show delay-1' : ''}`}>
          ...
          <div className={`next-step-box slide-right ${nextStepsVisible ? 'show delay-1' : ''}`}>
