...
const Testimonials = () => {
    ...
    return (
        <section className="testimonials-page-section" ref={testimonialsRef}>
            <h1
                className={`testimonials-page-title fade-up ${testimonialsVisible ? "show" : ""}`}
            >
                TESTIMONIALS
            </h1>
            <div
                className={`testimonials-page-images fade-up ${testimonialsVisible ? "show" : ""}`}
            >
                <img
                    src="https://i.imgur.com/2yG3qAq.png"
                    alt="Testimonial Graphic 2"
                    className="testimonials-page-image-larger"
                />
                <img
            ...
            <div
                className={`testimonials-page-carousel fade-up ${testimonialsVisible ? "show delay-2" : ""}`}
            >
                <Carousel
                    {...{
                        showArrows: true,
                        ...
                        showIndicators: true,
                        centerMode: false,
                        className: "testimonials-page-carousel-inner"
                    } as any}
                >
                    {isMobile ? (
                        testimonials.map((testimonial, index) => (
                            <div className="testimonials-page-cards" key={index}>
                                <div className="testimonials-page-card">
                                    <h3 className="testimonials-page-author">{testimonial.author}</h3>
                                    <div className="testimonials-page-stars">⭐⭐⭐⭐⭐</div>
                                    <p className="testimonials-page-text">{testimonial.text}</p>
                                </div>
                            </div>
                        ))
                    ) : (
                        Array.from({ length: Math.ceil(testimonials.length / 3) }).map((_, groupIndex) => (
                            <div className="testimonials-page-cards" key={groupIndex}>
                                {testimonials.slice(groupIndex * 3, groupIndex * 3 + 3).map((testimonial, index) => (
                                    <div key={index} className="testimonials-page-card">
                                        <h3 className="testimonials-page-author">{testimonial.author}</h3>
                                        <div className="testimonials-page-stars">⭐⭐⭐⭐⭐</div>
                                        <p className="testimonials-page-text">{testimonial.text}</p>
                                    </div>
